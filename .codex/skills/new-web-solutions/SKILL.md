...
2. Determine whether the customer uses the new Web Attribution solution or legacy PBA. For new Web Attribution, route independent web reporting to **My Dashboards > Templates > Web Analytics** and do not default to legacy PBA dashboards.
3. Load only the relevant reference files from `references/`.
4. Separate what is customer-safe from what is internal-only.
5. If the question is about current status or a recent Slack discussion, use the source map to fetch/search live Slack or Notion context before answering.
6. Draft the reply with a short diagnosis, practical next steps, and support article links.
7. Include clarifying questions only when needed to avoid a wrong answer, such as missing app ID, unified app ID, event name, date range, landing URL, CUID/cookie details, implementation method, or partner integration.
