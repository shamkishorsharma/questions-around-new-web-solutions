...
## Reporting Surface Routing

- **New Web Attribution, independent web metrics:** Use **My Dashboards > Templates > Web Analytics**. The template launched for Web Attribution beta accounts in July 2026.
- **Full event payloads and referrer-level analysis:** Use the Web End User Events Data Locker report. Web Analytics is an aggregated dashboard, not a row-level event log.
- **Cross-platform journeys:** Use the Cross-Platform dashboard only when the customer wants Product Line/CUID-based analysis across web, mobile, CTV, PC, or console.
- **Legacy PBA:** Use PBA dashboards and PBA raw reports only for customers explicitly working with the legacy PBA solution or validating a migration.
- For customers already on the new Web Attribution solution, do not recommend marking events as conversions in legacy PBA merely to make them visible.
- Prefer the new Web Attribution S2S endpoint (`/v2.0/s2s/inapps/app/web/{app_id}`) for the new solution. Treat `/v1/{bundle_id}/event` as the separate legacy PBA path, and do not encourage dual-sending the same production event to both endpoints.
- If a beta customer cannot see the Web Analytics template, verify current account enablement before redirecting them to PBA.
